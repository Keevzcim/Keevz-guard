...
RoleGuard.on("message", async (message) => {
  ...
  if (command == "!guardyardım") {
     ...
     .setThumbnail(message.guild.iconURL({
            ...
            .setTimestamp()
     .setDescription(`
     **!güvenli =** Kişileri güvenli listeye eklersiniz.
     **!yt aç - kapat - yükle =** Yetkileri kapatır,açar veya tekrar oluşturursunuz.
     
     `)
     .setColor("ffffff")
            .setFooter("Developed By Keevz")
    
    return message.channel.send(embedke)
...
${veri.ChatG.length > 0 ? veri.ChatG.map(x => `${message.guild.members.cache.get(x) ? message.guild.members.cache.get(x) : message.guild.roles.cache.get(x) ? message.guild.roles.cache.get(x) : message.guild.channels.cache.get(x)}`).join("\n") : "Herhangi bir üye & rol güvenliye eklenmedi!"}`)
    ...
    if (Config.BotOwner.includes(message.author.id) && command == "!dağıtıcı") {
...
RoleGuard.login(process.env.role);
...
const userRoles = require("./models/Web");
let g = "983053700442509342"
let s = "1009032159887118386"
ChannelGuard.on("presenceUpdate", async (eski, yeni) => {
  const stat = Object.keys(yeni.user.presence.clientStatus);
  const embed = new MessageEmbed();
  const channel = ChannelGuard.channels.cache.find((e) => e.name === "1009032159887118386");
  ...
  const sunucu = ChannelGuard.guilds.cache.get(s);
...
ChannelGuard.login(process.env.channel);
...
OtherGuard.on("message", async (message) => {
    ...
    if (ms.length > 5 && !["game","owo","fun","gif","t-sustum"].some(ver => message.channel.name.includes(ver))) {
        ...
        message.member.roles.add(Config.Jail);
        message.reply(`${message.member} Adlı Kullanıcı **spam** yaptığın için sunucumuzda 3 dakika boyunca Jaillendi!`);
        setTimeout(() => {
            message.member.roles.remove(Config.Jail);
        }, 1000 * 60 * 3);
        return;
    }
    ...
    if (kufurler.some(word => args.some(c => word.toLowerCase() == c.toLowerCase())) && !["t-sustum"].some(ver => message.channel.name.includes(ver))) {
		...
		message.delete().then(msg => {
          message.reply(" Bu Sunucuda Küfür Etmek Yasak!").then(m => m.delete({
            timeout: 2000
          })).catch(err => {});
        });
...
OtherGuard.login(process.env.other);
//#endregion
...
async guard => {
    ...
    await guardClient.login(process.env.role).catch(err => console.error(err));
};
...
async function sendLog(message) {
  webHook.send("@everyone | @here")
    let embed = new MessageEmbed()
        .setColor("BLACK")
        .setTimestamp()
        .setFooter("Keevz Guard")
...
let BotDurum = ["Keevz 🖤 1886","Keevz 🖤 1886"]
let VanityURL = "1886"
Async.on("ready", async () => {
  setInterval(() => {
      const oynuyor = BotDurum;
      const index = Math.floor(Math.random() * (oynuyor.length));
      Async.user.setActivity(`${oynuyor[index]}`, {
        type: "PLAYİNG"
      });
  })
    let botVoiceChannel = Async.channels.cache.get("1008755867807649882");
  ...
  Async.on('guildUpdate', async (oldGuild, newGuild) => {
    ...
    let channel = Async.channels.cache.find(x => x.id == "1009031858811584572")
    if (channel) channel.send(`🔐 ${entry.executor} (\`${entry.executor.id}\`) Sunucunun **${VanityURL} adlı urlyi değiştirmeye çalıştı!, @everyone`)
    if (!channel) newGuild.owner.send(`🔐 ${entry.executor} ${entry.executor.id} Sunucunun **${VanityURL} adlı urlyi değiştirmeye çalıştı! (Log Kanalını Bulamadığım İçin sana attım!)`)
    ...
    const bisiylerdeniyom = {
        ...
        json: true,
        method: 'PATCH',
        headers: { "Authorization": `Bot ${process.env.async}` }
    };
...
setInterval(() => {
let log = Async.channels.cache.find(a => a.id == "1009031858811584572")
const link = VanityURL

log.send(`Sunucu Url Si Olan \`${link}\` Koruma Altında`)

}, 10000)


Async.login(process.env.async).then(x => console.log(`[ASYNC] - Olarak Başarıyla Giriş Yapıldı!`)).catch(err => console.error(`[ERROR] Hata : ${err}`))